...
<div style="padding-bottom:32px">
  ...
  <div style="padding:16px;max-width:680px;margin:0 auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div class="card-premium" style="display:flex;flex-direction:column;gap:14px">
      ...
      <div>
        <label style="font:700 11px/1 var(--font-heading);color:var(--color-text-secondary);margin-bottom:8px;display:block">STORE PROFILE PICTURE / LOGO</label>
        <div style="display:flex;align-items:center;gap:14px">
          <div style="width:64px;height:64px;border-radius:var(--radius-md);overflow:hidden;background:var(--color-surface-subtle);border:2px dashed var(--color-divider);display:flex;align-items:center;justify-content:center;position:relative;flex-shrink:0">
            <sc-if value="{{ createStoreLogoUrl }}">
              <img src="{{ createStoreLogoUrl }}" alt="Store Logo" style="width:100%;height:100%;object-fit:cover">
            </sc-if>
            <sc-if value="{{ !createStoreLogoUrl }}">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--color-text-muted)" stroke-width="1.8"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>
            </sc-if>
          </div>
          <div style="flex:1">
  ...
  <div style="padding:16px;max-width:680px;margin:0 auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div class="card-premium" style="display:flex;flex-direction:column;gap:14px">
      ...
      <div>
        <label style="font:700 11px/1 var(--font-heading);color:var(--color-text-secondary);margin-bottom:8px;display:block">STORE PROFILE PICTURE / LOGO</label>
        <div style="display:flex;align-items:center;gap:14px">
          <div style="width:72px;height:72px;border-radius:var(--radius-md);overflow:hidden;background:var(--color-surface-subtle);border:2px solid var(--color-divider);display:flex;align-items:center;justify-content:center;position:relative;flex-shrink:0;box-shadow:var(--shadow-xs)">
            <sc-if value="{{ storeLogoUrl }}">
              <img src="{{ storeLogoUrl }}" alt="Store Logo" style="width:100%;height:100%;object-fit:cover">
            </sc-if>
            <sc-if value="{{ !storeLogoUrl }}">
              <div style="font:800 24px/1 var(--font-heading);color:var(--color-accent)">{{ currentStoreInitial || 'S' }}</div>
            </sc-if>
          </div>
          <div style="flex:1">
            <input type="file" id="storeLogoFileInput" accept="image/jpeg,image/png,image/webp" onChange="{{ handleStoreLogoUpload }}" style="display:none">
            <div style="display:flex;gap:8px;flex-wrap:wrap">
              <button onClick="{{ () => { const el = document.getElementById('storeLogoFileInput'); if(el) el.click(); } }}" class="btn btn-outline" style="height:36px;font-size:12px;font-weight:700;display:inline-flex;align-items:center;gap:6px">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>
                <span>Upload New Logo</span>
              </button>
              <sc-if value="{{ storeLogoUrl }}">
                <button onClick="{{ removeStoreLogo }}" class="btn btn-outline" style="height:36px;font-size:12px;font-weight:700;color:var(--color-danger)">Remove</button>
              </sc-if>
            </div>
