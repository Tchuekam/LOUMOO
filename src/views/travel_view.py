...
<div style="padding-bottom:70px">
  ...
  <div style="padding:16px 16px;max-width:1040px;margin:0 auto;display:flex;flex-direction:column;gap:20px">
    ...
    <div class="travel-search-widget">
      ...
      <div class="flight-route-row">
        <div style="background:var(--color-surface-subtle);border:1px solid var(--color-border-subtle);border-radius:var(--radius-md);padding:9px 12px;min-width:0">
          ...
          <input value="{{ travelFrom }}" onInput="{{ updateTravelFrom }}" aria-label="Departure location" style="width:100%;border:none;background:transparent;padding:0;font:700 13.5px/1.2 var(--font-heading);color:var(--color-text);outline:none" />
        </div>
        ...
        <div style="background:var(--color-surface-subtle);border:1px solid var(--color-border-subtle);border-radius:var(--radius-md);padding:9px 12px;min-width:0">
          ...
          <input value="{{ travelTo }}" onInput="{{ updateTravelTo }}" aria-label="Arrival location" style="width:100%;border:none;background:transparent;padding:0;font:700 13.5px/1.2 var(--font-heading);color:var(--color-text);outline:none" />
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
        <div style="background:var(--color-surface-subtle);border:1px solid var(--color-border-subtle);border-radius:var(--radius-md);padding:9px 12px">
          ...
          <input type="date" value="{{ travelDate }}" onInput="{{ updateTravelDate }}" min="{{ todayIso }}" aria-label="Departure date" style="width:100%;border:none;background:transparent;padding:0;font:600 12.5px/1.2 var(--font-body);color:var(--color-text);outline:none" />
        </div>
    ...
    <sc-if value="{{ travelLandingLoading }}">
      <div class="card-premium" style="padding:24px;text-align:center;color:var(--color-text-secondary)">Loading live travel inventory…</div>
    </sc-if>
    <sc-if value="{{ travelLandingError }}">
      <div class="card-premium" style="padding:20px;color:var(--color-danger)">{{ travelLandingError }}</div>
    </sc-if>
    <sc-if value="{{ travelLandingLoaded && !travelLandingLoading && !travelLandingError && !travelLandingItems.length }}">
      <div class="card-premium" style="padding:24px;text-align:center;color:var(--color-text-secondary)">No travel inventory is currently available.</div>
    </sc-if>
    <sc-if value="{{ travelLandingLoaded && !travelLandingLoading && !travelLandingError && travelLandingItems.length }}">
      <div>
        <h3 style="margin:0 0 10px;font-size:15px;font-weight:800;color:var(--color-text)">Live travel inventory</h3>
        <div style="display:flex;flex-direction:column;gap:10px">
          <sc-for list="{{ travelLandingItems }}" as="item">
            <div onClick="{{ () => selectTravelResult(item) }}" class="card-premium" style="padding:14px;display:flex;justify-content:space-between;align-items:center;gap:12px;cursor:pointer">
              <div><div style="font:800 13px/1.2 var(--font-heading)">{{ item.provider }}</div><div style="font:500 11px/1.2 var(--font-body);color:var(--color-text-secondary);margin-top:4px">{{ item.origin }} → {{ item.destination }} · {{ item.type }}</div></div>
              <div style="font:800 14px/1 var(--font-heading);color:var(--color-accent);white-space:nowrap">{{ item.currency }} {{ item.price }}</div>
            </div>
          </sc-for>
        </div>
      </div>
      ...
      <div class="travel-rail" style="display:flex;flex-direction:row;flex-wrap:nowrap;gap:12px;overflow-x:auto;overflow-y:hidden;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:4px 2px 10px;margin:0 -16px;padding-left:16px;padding-right:16px">
        ...
        <div onClick="{{ on.travelPackages }}" class="travel-card-compact" style="flex:0 0 185px;width:185px;max-width:220px;scroll-snap-align:start;background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius-lg);overflow:hidden;display:flex;flex-direction:column;cursor:pointer;box-shadow:var(--shadow-xs);box-sizing:border-box">
          <div class="card-img-wrap" style="height:110px;max-height:110px;width:100%;position:relative;overflow:hidden;background:var(--color-surface-subtle)">
            <img src="./Assets/Travel&Hotel/residence-jully-kribi.jpg" alt="Kribi Beach & Lobé Waterfalls" style="width:100%;height:100%;max-height:110px;object-fit:cover;display:block" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Residence%20JULLY%20Kribi.jfif'">
        ...
        <div onClick="{{ on.travelPackages }}" class="travel-card-compact" style="flex:0 0 185px;width:185px;max-width:220px;scroll-snap-align:start;background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius-lg);overflow:hidden;display:flex;flex-direction:column;cursor:pointer;box-shadow:var(--shadow-xs);box-sizing:border-box">
          <div class="card-img-wrap" style="height:110px;max-height:110px;width:100%;position:relative;overflow:hidden;background:var(--color-surface-subtle)">
            <img src="./Assets/Travel&Hotel/hotel-du-phare-kribi.jpg" alt="Limbe Botanic & Mount Cameroon" style="width:100%;height:100%;max-height:110px;object-fit:cover;display:block" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Hotel%20du%20Phare%20(Kribi%2C%20Cameroun)%20_%20tarifs%202019%20mis%E2%80%A6.jfif'">
        ...
        <div onClick="{{ on.travelPackages }}" class="travel-card-compact" style="flex:0 0 185px;width:185px;max-width:220px;scroll-snap-align:start;background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius-lg);overflow:hidden;display:flex;flex-direction:column;cursor:pointer;box-shadow:var(--shadow-xs);box-sizing:border-box">
          <div class="card-img-wrap" style="height:110px;max-height:110px;width:100%;position:relative;overflow:hidden;background:var(--color-surface-subtle)">
            <img src="./Assets/Travel&Hotel/residence-jully-kribi.jpg" alt="Dja Biosphere Reserve" style="width:100%;height:100%;max-height:110px;object-fit:cover;display:block" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Residence%20JULLY%20Kribi.jfif'">
            <span style="position:absolute;top:6px;left:6px;background:rgba(0,0,0,0.65);color:#fff;padding:2px 6px;border-radius:var(--radius-pill);font:800 8.5px/1 var(--font-heading);letter-spacing:.04em">3D / 2N</span>
          </div>
          <div class="card-info">
        ...
        <div onClick="{{ on.travelPackages }}" class="travel-card-compact" style="flex:0 0 185px;width:185px;max-width:220px;scroll-snap-align:start;background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius-lg);overflow:hidden;display:flex;flex-direction:column;cursor:pointer;box-shadow:var(--shadow-xs);box-sizing:border-box">
          <div class="card-img-wrap" style="height:110px;max-height:110px;width:100%;position:relative;overflow:hidden;background:var(--color-surface-subtle)">
            <img src="./Assets/Travel&Hotel/hotel-du-phare-kribi.jpg" alt="Rhumsiki Lunar Peaks" style="width:100%;height:100%;max-height:110px;object-fit:cover;display:block" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Hotel%20du%20Phare%20(Kribi%2C%20Cameroun)%20_%20tarifs%202019%20mis%E2%80%A6.jfif'">
      ...
      <div class="travel-rail" style="display:flex;flex-direction:row;flex-wrap:nowrap;gap:12px;overflow-x:auto;overflow-y:hidden;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:4px 2px 10px;margin:0 -16px;padding-left:16px;padding-right:16px">
        ...
        <div onClick="{{ openHotelDetail }}" class="hotel-card-compact" style="flex:0 0 195px;width:195px;max-width:230px;scroll-snap-align:start;background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius-lg);overflow:hidden;display:flex;flex-direction:column;cursor:pointer;box-shadow:var(--shadow-xs);box-sizing:border-box">
          <div class="card-img-wrap" style="height:112px;max-height:112px;width:100%;position:relative;overflow:hidden;background:var(--color-surface-subtle)">
            <img src="./Assets/Travel&Hotel/krystal-palace-douala.jpg" alt="Krystal Palace Hotel Douala" style="width:100%;height:100%;max-height:112px;object-fit:cover;display:block" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Krystal%20Palace%20Hotel%20Douala.jfif'">
        ...
        <div onClick="{{ openHotelDetail }}" class="hotel-card-compact" style="flex:0 0 195px;width:195px;max-width:230px;scroll-snap-align:start;background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius-lg);overflow:hidden;display:flex;flex-direction:column;cursor:pointer;box-shadow:var(--shadow-xs);box-sizing:border-box">
          <div class="card-img-wrap" style="height:112px;max-height:112px;width:100%;position:relative;overflow:hidden;background:var(--color-surface-subtle)">
            <img src="./Assets/Travel&Hotel/hotel-du-phare-kribi.jpg" alt="Hôtel du Phare Kribi" style="width:100%;height:100%;max-height:112px;object-fit:cover;display:block" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Hotel%20du%20Phare%20(Kribi%2C%20Cameroun)%20_%20tarifs%202019%20mis%E2%80%A6.jfif'">
        ...
        <div onClick="{{ openHotelDetail }}" class="hotel-card-compact" style="flex:0 0 195px;width:195px;max-width:230px;scroll-snap-align:start;background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius-lg);overflow:hidden;display:flex;flex-direction:column;cursor:pointer;box-shadow:var(--shadow-xs);box-sizing:border-box">
          <div class="card-img-wrap" style="height:112px;max-height:112px;width:100%;position:relative;overflow:hidden;background:var(--color-surface-subtle)">
            <img src="./Assets/Travel&Hotel/residence-jully-kribi.jpg" alt="Résidence Jully Kribi" style="width:100%;height:100%;max-height:112px;object-fit:cover;display:block" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Residence%20JULLY%20Kribi.jfif'">
      ...
      <sc-if value="{{ !busSchedulesLoading && !busSchedulesError && filteredBusSchedules.length }}">
        <sc-for list="{{ filteredBusSchedules }}" as="bus">
          <div class="card-premium" style="padding:22px;border-radius:var(--radius-lg);box-shadow:var(--shadow-sm)">
            ...
            <sc-if value="{{ bus.isSelectedSchedule }}">
            ...
            <sc-if value="{{ !bus.isSelectedSchedule }}">
              <div style="border-top:1px solid var(--color-divider);padding-top:14px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
                <span style="font:500 12px/1 var(--font-body);color:var(--color-text-secondary)">Direct Operator Route · Real-time Seat Map</span>
                <button onClick="{{ () => selectBusSchedule(bus) }}" class="btn btn-secondary" style="height:38px;padding:0 18px;font-size:12.5px;font-weight:800;border-radius:var(--radius-pill)">
                  Select Seats &amp; Reserve <span>↓</span>
                </button>
              </div>
            </sc-if>
          </div>
...
<div style="padding-bottom:60px">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;background:var(--color-surface-glass);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-bottom:1px solid var(--color-divider);position:sticky;top:0;z-index:30">
    <div style="display:flex;align-items:center;gap:14px">
      ...
      <div>
        <h4 style="margin:0;font-size:16.5px;font-weight:800;color:var(--color-text);letter-spacing:-.01em">{{ travelRouteLabel || (travelFrom + ' → ' + travelTo) }}</h4>
        <div style="font:500 12px/1 var(--font-body);color:var(--color-text-secondary);margin-top:2px">{{ travelDate }} · {{ travelSearchResults.length }} Available Route(s)</div>
      </div>
    </div>
  </div>
  ...
  <div style="padding:24px 20px;max-width:1120px;margin:0 auto">
    ...
    <div style="display:grid;grid-template-columns:minmax(0, 1.25fr) minmax(320px, 0.95fr);gap:24px;align-items:start" class="travel-desktop-split">
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        <sc-if value="{{ travelSearchLoading }}">
          <div class="card-premium" style="padding:28px;text-align:center;color:var(--color-text-secondary)">Searching live operator inventory…</div>
        </sc-if>
        <sc-if value="{{ travelSearchError }}">
          <div class="card-premium" style="padding:20px;color:var(--color-danger)">{{ travelSearchError }}</div>
        </sc-if>
        <sc-if value="{{ travelSearchDone && !travelSearchLoading && !travelSearchError && !travelSearchResults.length }}">
          <div class="card-premium" style="padding:28px;text-align:center;color:var(--color-text-secondary)">No travel options were found for this route and date.</div>
        </sc-if>
        <sc-if value="{{ travelSearchDone && !travelSearchLoading && !travelSearchError && travelSearchResults.length }}">
          <sc-for list="{{ travelSearchResults }}" as="result">
            <div onClick="{{ () => selectTravelResult(result) }}" class="card-premium" style="cursor:pointer;padding:22px;border-radius:var(--radius-lg);box-shadow:var(--shadow-sm)">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:12px">
                <div>
                  <div style="font:800 15px/1 var(--font-heading);color:var(--color-text)">{{ result.provider }}</div>
                  <div style="font:600 11px/1.2 var(--font-body);color:var(--color-text-secondary);margin-top:5px;text-transform:uppercase">{{ result.type }} · {{ result.className }}</div>
                </div>
                <span style="font:800 20px/1 var(--font-heading);color:var(--color-accent)">{{ result.currency }} {{ result.price }}</span>
              </div>
              <div style="display:flex;align-items:center;justify-content:space-between;margin:18px 0;padding:14px 18px;background:var(--color-surface-subtle);border-radius:var(--radius-md)">
                <div><div style="font:800 17px/1 var(--font-heading)">{{ result.departure }}</div><div style="font:500 12px/1.3 var(--font-body);color:var(--color-text-secondary);margin-top:4px">{{ result.origin }}</div></div>
                <div style="flex:1;text-align:center;padding:0 12px;color:var(--color-text-muted);font:700 11px/1.2 var(--font-body)">{{ result.duration }}</div>
                <div style="text-align:right"><div style="font:800 17px/1 var(--font-heading)">{{ result.arrival }}</div><div style="font:500 12px/1.3 var(--font-body);color:var(--color-text-secondary);margin-top:4px">{{ result.destination }}</div></div>
              </div>
              <div style="display:flex;justify-content:space-between;border-top:1px solid var(--color-divider);padding-top:14px;font-size:12px;color:var(--color-text-secondary)">
                <span>{{ result.availableUnits }} seats/units available · Rating {{ result.rating }}</span>
                <button class="btn btn-primary" style="height:36px;padding:0 18px;font-size:12px;font-weight:800;border-radius:var(--radius-pill)">Select</button>
              </div>
            </div>
          </sc-for>
        </sc-if>
        <sc-if value="{{ !travelSearchDone }}">
...
<div style="padding-bottom:60px">
  ...
  <div style="display:flex;align-items:center;gap:14px;padding:14px 20px;background:var(--color-surface-glass);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-bottom:1px solid var(--color-divider);position:sticky;top:0;z-index:30">
    ...
    <div style="flex:1;min-width:0">
      <sc-if value="{{ selectedTravelResult }}">
        <h4 style="margin:0;font-size:16.5px;font-weight:800;letter-spacing:-.01em;color:var(--color-text)">{{ selectedTravelResult.provider }}</h4>
        <div style="font:500 12px/1 var(--font-body);color:var(--color-text-secondary);margin-top:2px">{{ selectedTravelResult.origin }} → {{ selectedTravelResult.destination }} · {{ selectedTravelResult.className || selectedTravelResult.type }}</div>
      </sc-if>
      <sc-if value="{{ !selectedTravelResult }}">
        <h4 style="margin:0;font-size:16.5px;font-weight:800;letter-spacing:-.01em;color:var(--color-text)">Air France Flight AF949</h4>
    ...
    <sc-if value="{{ selectedTravelResult }}">
      <div class="card-premium" style="padding:24px;border-radius:var(--radius-xl);box-shadow:var(--shadow-sm)">
        <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--color-divider);padding-bottom:16px;margin-bottom:18px">
          <div>
            <div style="display:flex;align-items:center;gap:8px">
              <span style="font:800 16.5px/1 var(--font-heading);color:var(--color-text)">{{ selectedTravelResult.provider }}</span>
              <span style="background:var(--color-accent-100);color:var(--color-accent);padding:2px 8px;border-radius:var(--radius-pill);font:800 10px/1 var(--font-heading);text-transform:uppercase">{{ selectedTravelResult.type }}</span>
            </div>
            <div style="font:500 12px/1.2 var(--font-body);color:var(--color-text-secondary);margin-top:4px">{{ selectedTravelResult.className || 'VIP Direct Transit' }} · Escrow Protected</div>
          </div>
          <div style="text-align:right">
            <span style="font:800 22px/1 var(--font-heading);color:var(--color-accent)">{{ selectedTravelResult.currency }} {{ selectedTravelResult.price }}</span>
            <div style="font:500 11px/1 var(--font-body);color:var(--color-success);margin-top:2px">All Taxes &amp; Fees Included</div>
          </div>
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:16px;font-size:13.5px">
          <div style="display:flex;gap:14px;align-items:flex-start">
            ...
            <div>
              <div style="font-weight:800;font-size:16px;color:var(--color-text)">{{ selectedTravelResult.departure }} · {{ selectedTravelResult.origin }}</div>
              <div style="color:var(--color-text-secondary);font-size:12.5px;margin-top:2px">Departure Hub · Verified Boarding Station</div>
            </div>
          </div>
          ...
          <div style="height:32px;border-left:2px dashed var(--color-divider);margin-left:5px;padding-left:21px;display:flex;align-items:center;color:var(--color-text-muted);font-size:12px;font-weight:600">
            {{ selectedTravelResult.duration }} Transit Duration · Non-stop Official Corridor
          </div>

          <div style="display:flex;gap:14px;align-items:flex-start">
            <div style="width:12px;height:12px;border-radius:50%;background:var(--color-success);margin-top:4px;flex-shrink:0"></div>
            <div>
              <div style="font-weight:800;font-size:16px;color:var(--color-text)">{{ selectedTravelResult.arrival }} · {{ selectedTravelResult.destination }}</div>
              <div style="color:var(--color-text-secondary);font-size:12.5px;margin-top:2px">Arrival Destination · Express Baggage Claim</div>
            </div>
          </div>
        </div>
        ...
        <div style="margin-top:20px;padding:14px 18px;background:var(--color-surface-subtle);border-radius:var(--radius-md);border:1px solid var(--color-border-subtle);display:flex;justify-content:space-between;align-items:center">
          <span style="font-size:12.5px;color:var(--color-text-secondary)">Complimentary luggage allowance + instant digital boarding pass</span>
          <span style="font:800 12px/1 var(--font-heading);color:var(--color-success)">OFFICIAL</span>
        </div>
      </div>
    </sc-if>
    <sc-if value="{{ !selectedTravelResult }}">
...
<div style="padding-bottom:60px">
  ...
  <div style="padding:24px 20px;max-width:720px;margin:0 auto;display:flex;flex-direction:column;gap:18px">
    ...
    <div style="padding:18px 22px;background:var(--color-surface-subtle);border:1px solid var(--color-border-subtle);border-radius:var(--radius-lg);display:flex;justify-content:space-between;align-items:center">
      ...
      <div style="text-align:right">
        <div style="font:800 18px/1 var(--font-heading);color:var(--color-accent)">{{ selectedTravelResult.currency }} {{ selectedTravelResult.price }}</div>
...
<div style="padding:24px 20px 48px;max-width:560px;margin:0 auto">
    ...
    <div class="boarding-pass" style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius-xl);overflow:hidden;box-shadow:var(--shadow-xl)">
      ...
      <div class="pass-header" style="background:linear-gradient(135deg, #0b2e5c 0%, #123f7a 55%, #1a5fb4 100%);padding:22px 24px;color:#fff">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div style="display:flex;align-items:center;gap:12px">
            ...
            <div>
              <div style="font:800 15px/1 var(--font-heading);letter-spacing:.01em">{{ ticketOperator || 'LOUMOO Travel' }}</div>
              <div style="font:500 11px/1.2 var(--font-body);opacity:0.85;margin-top:3px">Verified Digital Transit Pass</div>
            </div>
          </div>
          <div style="text-align:right">
            <div style="font:800 10.5px/1 var(--font-heading);letter-spacing:.14em;opacity:0.85">BOARDING PASS</div>
            <div style="font:800 11px/1 var(--font-mono);margin-top:5px;background:rgba(255,255,255,0.2);padding:4px 10px;border-radius:var(--radius-pill)">VIP / FIRST</div>
          </div>
        </div>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:22px">
          ...
          <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;padding:0 8px">
            <div style="font:600 10px/1 var(--font-body);opacity:0.85">Direct Corridor</div>
      ...
      <div class="pass-body" style="padding:22px 24px;border-bottom:2px dashed var(--color-divider)">
        <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:18px 16px;font-size:13px">
          ...
          <div>
            <div style="color:var(--color-text-muted);font-weight:700;font-size:10px;letter-spacing:.06em">OPERATOR &amp; SERVICE</div>
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:18px;padding-top:16px;border-top:1px solid var(--color-divider)">
          ...
          <div>
            <div style="color:var(--color-text-muted);font-weight:700;font-size:9.5px;letter-spacing:.05em">GATE / PLATFORM</div>
      ...
      <div class="pass-qr-wrap" style="padding:22px 24px;background:var(--color-surface)">
        <div style="display:flex;align-items:center;gap:18px">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font:700 10.5px/1 var(--font-mono);color:var(--color-text-muted);margin-top:10px;letter-spacing:.12em">ETKT {{ ticketPnr }} · VERIFIED CEMAC PASS</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- Empty State When No Active Ticket Exists -->
  <sc-if value="{{ !hasActiveTicket }}">
    <div class="card-premium" style="text-align:center;padding:48px 24px;border-radius:var(--radius-xl);box-shadow:var(--shadow-md);display:flex;flex-direction:column;align-items:center;gap:16px">
      <div style="width:64px;height:64px;border-radius:50%;background:rgba(0,122,255,0.08);border:1px solid rgba(0,122,255,0.2);display:flex;align-items:center;justify-content:center;color:var(--color-accent)">
      ...
      <div>
        <h3 style="margin:0 0 6px;font-size:18px;font-weight:800;color:var(--color-text)">No Active Boarding Pass</h3>
        <p style="margin:0;font-size:13px;color:var(--color-text-secondary);max-width:360px;line-height:1.4">
          Your verified digital boarding passes, VIP bus reservations, and QR tickets will appear here once booked.
        </p>
      </div>
      <button onClick="{{ on.travel }}" class="btn btn-primary" style="height:44px;padding:0 24px;font-size:13.5px;font-weight:800;border-radius:var(--radius-pill);box-shadow:var(--shadow-glow-blue);margin-top:6px">
        Explore &amp; Book Travel <span>→</span>
      </button>
    </div>
  </sc-if>
</div>
  ...
  <div style="padding:24px 20px;max-width:1120px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fit, minmax(320px, 1fr));gap:20px">
    ...
    <div class="card-premium" style="padding:0;overflow:hidden;border-radius:var(--radius-lg);box-shadow:var(--shadow-sm)">
      <div style="height:210px;position:relative">
        <img src="./Assets/Travel&Hotel/residence-jully-kribi.jpg" alt="Kribi Beach & Lobé Falls Escape" style="width:100%;height:100%;object-fit:cover" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Residence%20JULLY%20Kribi.jfif'">
      ...
      <div style="padding:18px 20px;display:flex;justify-content:space-between;align-items:center;background:var(--color-surface)">
        ...
        <button onClick="{{ selectPackageKribi }}" class="btn btn-primary" style="height:38px;padding:0 18px;font-size:12.5px;font-weight:800;border-radius:var(--radius-pill)">Reserve Package</button>
      </div>
    </div>

    <div class="card-premium" style="padding:0;overflow:hidden;border-radius:var(--radius-lg);box-shadow:var(--shadow-sm)">
      <div style="height:210px;position:relative">
        <img src="./Assets/Travel&Hotel/hotel-du-phare-kribi.jpg" alt="Limbe Botanic & Mount Cameroon Hike" style="width:100%;height:100%;object-fit:cover" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Hotel%20du%20Phare%20(Kribi%2C%20Cameroun)%20_%20tarifs%202019%20mis%E2%80%A6.jfif'">
        <div style="position:absolute;inset:0;background:linear-gradient(180deg, rgba(0,0,0,0.1) 40%, rgba(0,0,0,0.8) 100%)"></div>
      ...
      <div style="padding:18px 20px;display:flex;justify-content:space-between;align-items:center;background:var(--color-surface)">
        ...
        <button onClick="{{ selectPackageLimbe }}" class="btn btn-primary" style="height:38px;padding:0 18px;font-size:12.5px;font-weight:800;border-radius:var(--radius-pill)">Reserve Package</button>
      </div>
    </div>

    <div class="card-premium" style="padding:0;overflow:hidden;border-radius:var(--radius-lg);box-shadow:var(--shadow-sm)">
      <div style="height:210px;position:relative">
        <img src="./Assets/Travel&Hotel/hotel-le-relais-nord.jpg" alt="Rhumsiki Peaks & Kapsiki Expedition" style="width:100%;height:100%;object-fit:cover" onerror="this.onerror=null;this.src='./Assets/Travel%26Hotel/Hotel%20le%20relais%20-%20Nord%20Cameroun.jfif'">
        <div style="position:absolute;inset:0;background:linear-gradient(180deg, rgba(0,0,0,0.1) 40%, rgba(0,0,0,0.8) 100%)"></div>
      ...
      <div style="padding:18px 20px;display:flex;justify-content:space-between;align-items:center;background:var(--color-surface)">
        ...
        <button onClick="{{ selectPackageRhumsiki }}" class="btn btn-primary" style="height:38px;padding:0 18px;font-size:12.5px;font-weight:800;border-radius:var(--radius-pill)">Reserve Package</button>
      </div>
    </div>
  ...
  <div style="padding:24px 20px;max-width:720px;margin:0 auto;display:flex;flex-direction:column;gap:18px">
    ...
    <div class="card-premium" style="padding:22px;border-radius:var(--radius-lg);box-shadow:var(--shadow-sm)">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
        <div>
          <div style="font:800 16px/1.2 var(--font-heading);color:var(--color-text)">{{ visaCountryLabel }}</div>
          <div style="font:500 12px/1 var(--font-body);color:var(--color-text-secondary);margin-top:3px">Applicant: {{ visaApplicantName }} · Ref: {{ visaApplicationRef }}</div>
        </div>
