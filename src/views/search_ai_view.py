...
<div style="padding-bottom:32px">
  ...
  <div style="padding:16px;max-width:960px;margin:0 auto">
    ...
    <!-- ── LOADING ─────────────────────────────────────────────── -->
    <sc-if value="{{ searchShowLoading }}">
      <div style="font:800 12px/1 var(--font-heading);letter-spacing:.08em;color:var(--color-text-muted);text-transform:uppercase;margin-bottom:12px">Searching…</div>
      <div style="display:flex;flex-direction:column;gap:10px">
        <div class="card-premium" style="display:flex;align-items:center;gap:14px;padding:12px 16px">
          <div class="ph" style="width:54px;height:54px;border-radius:var(--radius-sm);flex-shrink:0"></div>
          <div style="flex:1"><div class="ph" style="height:14px;width:62%;border-radius:6px"></div><div class="ph" style="height:11px;width:40%;border-radius:6px;margin-top:8px"></div></div>
        </div>
        <div class="card-premium" style="display:flex;align-items:center;gap:14px;padding:12px 16px">
          <div class="ph" style="width:54px;height:54px;border-radius:var(--radius-sm);flex-shrink:0"></div>
          <div style="flex:1"><div class="ph" style="height:14px;width:52%;border-radius:6px"></div><div class="ph" style="height:11px;width:34%;border-radius:6px;margin-top:8px"></div></div>
        </div>
        <div class="card-premium" style="display:flex;align-items:center;gap:14px;padding:12px 16px">
          <div class="ph" style="width:54px;height:54px;border-radius:var(--radius-sm);flex-shrink:0"></div>
          <div style="flex:1"><div class="ph" style="height:14px;width:70%;border-radius:6px"></div><div class="ph" style="height:11px;width:44%;border-radius:6px;margin-top:8px"></div></div>
        </div>
      </div>
    </sc-if>

    <!-- ── ERROR ───────────────────────────────────────────────── -->
    <sc-if value="{{ searchShowError }}">
      <div style="text-align:center;padding:48px 24px">
        <div style="font:700 16px/1.3 var(--font-heading);color:var(--color-text);margin-bottom:8px">{{ searchError }}</div>
        <div style="font:400 13px/1.5 var(--font-body);color:var(--color-text-secondary);margin-bottom:18px">Please check your connection and try again.</div>
        <button onClick="{{ submitSearch }}" class="btn btn-primary" style="height:44px;padding:0 22px;font-size:13px">Try again</button>
      </div>
    </sc-if>
    ...
    <sc-if value="{{ searchShowResults }}">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ...
        <button onClick="{{ () => openProduct(prod.id) }}" class="card-premium" style="display:flex;align-items:center;gap:14px;padding:12px 16px;text-align:left;cursor:pointer;width:100%" aria-label="View listing">
          <div class="ph" style="width:54px;height:54px;border-radius:var(--radius-sm);flex-shrink:0;position:relative;overflow:hidden">
            <sc-if value="{{ prod.hasImage }}"><img src="{{ prod.imageUrl }}" alt="{{ prod.title }}" loading="lazy" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover"></sc-if>
          </div>
          <div style="flex:1;min-width:0">
            <div style="font:700 14px/1.2 var(--font-heading);color:var(--color-text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ prod.title }}</div>
            <div style="font:400 11.5px/1 var(--font-body);color:var(--color-text-secondary);margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ prod.storeLabel }}</div>
          </div>
          <div style="text-align:right;flex-shrink:0">
            <div style="font:800 15px/1 var(--font-heading);color:var(--color-accent)">{{ prod.priceLabel }}</div>
            <sc-if value="{{ prod.ratingLabel }}"><span style="font:600 10px/1 var(--font-body);color:var(--color-text-secondary)">{{ prod.ratingLabel }}</span></sc-if>
          </div>
        </button>
        </sc-if>
        </sc-for>
    ...
    <!-- ── DEFAULT (no active query): popular searches ─────────── -->
    <sc-if value="{{ searchShowDefault }}">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px">
        <button onClick="{{ () => runSearch('MacBook') }}" class="tag tag-neutral">
        ...
        <button onClick="{{ () => runSearch('Sony') }}" class="tag tag-neutral">
        ...
        <button onClick="{{ () => runSearch('iPhone') }}" class="tag tag-neutral">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect width="14" height="20" x="5" y="2" rx="2" ry="2"/><path d="M12 18h.01"/></svg>
          <span>iPhone</span>
        </button>
